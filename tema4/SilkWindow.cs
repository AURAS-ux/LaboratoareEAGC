...
public class SilkWindow
{
    ...
    private void AddNewPoint(Vector2 pos)
    {
        ...
        //validare puncte pentru curba lagrange
        if(_points.Count > 0 && normalizedX < _points[^1][0])
        {
            _logger.Warning("Points must be added in increasing X order for Lagrange interpolation. Point not added.");
            return;
        }
    ...
    private void OnInputMouseOnMouseDown(IMouse mouse, MouseButton button)
    {
        ...
        if (button == MouseButton.Middle)
        {
            if(_points.Count < 2)
            {
                _logger.Warning("At least two points are required to perform Lagrange interpolation.");
                return;
            }
            _logger.Information("Middle mouse button clicked. Performing Lagrange interpolation.");
            var lagrangePoints = LagrangHelper.ComputeLagrangeCurve(_points);
            _points = lagrangePoints;
            _logger.Information("Lagrange interpolation completed. Updating vertex buffer.");
            UpdateVertexBuffer();
        }
    }
